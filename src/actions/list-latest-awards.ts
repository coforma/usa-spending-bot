...
import { SlackAction } from "../types/SlackAction.js";

// create a command to list 5 latest awards from usa spending api for the given recipient id
export const listLatestAwardsAction: SlackAction = async ({ body, client, ack, logger }) => {
  // Acknowledge command request
  await ack();

  const shortName: string = body.actions[0].value.trim() || "";
